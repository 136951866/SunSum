...
@interface SSStarControl()
@property (nonatomic, strong) SSStarButton *currentStar;
...
- (void)setupView {
    for (NSInteger index = 0; index < self.numberOfStars; index++) {
        SSStarButton *starButton = [SSStarButton.alloc initWithSize:self.starSize];
...
- (void)layoutSubviews {
    ...
    for (NSInteger index = 0; index < self.numberOfStars; index ++) {
        SSStarButton *starButton =  [self starForTag:index];
...
- (SSStarButton *)starForPoint:(CGPoint)point {
    for (NSInteger i = 0; i < self.numberOfStars; i++) {
        SSStarButton *starButton = [self starForTag:i];
...
- (SSStarButton *)starForTag:(NSInteger)tag {
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    SSStarButton *pressedStar = [self starForPoint:point];
    if (pressedStar) {
