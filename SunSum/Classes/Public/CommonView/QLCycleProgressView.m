...
@property (nonatomic, strong) UILabel *allTask;
...
- (void)backgroundCyclePath {
    ...
    CGFloat radius = 90;
    CGFloat startA =  M_PI_2*3;  //设置进度条起点位置
    CGFloat endA = M_PI * 2+M_PI_2*3;  //设置进度条终点位置
    ...
    _backBorderLayer.lineWidth = 18;//线的宽度
    ...
    [self addSubview:self.allTask];
    _countJump.frame = CGRectMake((self.width - 60)/2, self.centerY-(27), 60, 27);
    _allTask.frame = CGRectMake((self.width-90)/2, _countJump.bottom+5, 90, 14);
...
- (void)setProgress:(CGFloat)progress {
    ...
    CGFloat radius = 90;
    CGFloat startA =  M_PI_2*3;  //设置进度条起点位置
    CGFloat endA = M_PI_2*3+M_PI * 2 * _progress;  //设置进度条终点位置
...
- (UILabel *)countJump {
    if (!_countJump) {
        ...
        _countJump.textColor = [UIColor blackColor];
...
- (UILabel *)allTask {
    if (!_allTask) {
        _allTask = [[UILabel alloc]init];
        _allTask.textColor = [UIColor colorWithHexString:@"999999"];
        _allTask.font = [UIFont systemFontOfSize:11];
        _allTask.textAlignment = NSTextAlignmentCenter;
    }
    return _allTask;
}
