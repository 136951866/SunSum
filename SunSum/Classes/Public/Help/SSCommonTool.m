...
+ (void)initAppSomeThing{
    ...
    if(!kMeUnNilStr(frist).length && [SSUserInfoModel isLogin]){
...
+ (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion{
    SSNavigationVC *nav = [[SSNavigationVC alloc]initWithRootViewController:viewControllerToPresent];
...
+ (void)doNavigationWithEndLocation:(NSArray *)endLocation{
    ...
    SSCustomActionSheet *sheet = [[SSCustomActionSheet alloc]initWithTitles:arrTitles];
...
+ (void)saveImg:(UIImage *)img{
    ...
    [library saveImage:img toAlbum:kSSAppName withCompletionBlock:^(NSError *error) {
        ...
        if (!error) {
            [SSShowViewTool SHOWHUDWITHHUD:HUD test:@"已保存到本地相册"];
        }else{
            [HUD hideAnimated:YES];
            [[[UIAlertView alloc]initWithTitle:@"无法保存" message:[NSString stringWithFormat:@"请在iPhone的“设置-隐私-照片”选项中，允许%@访问你的照片",kSSAppName] delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil] show];
        }
    }];
    return;
}
...
+ (void)newCheckVersion{
    [SSPublicNetWorkTool postGetNewAPPVersionWithSuccessBlock:^(ZLRequestResponse *responseObject) {
        ...
        switch (stat) {
            ...
            case 2:
            {
                ...
                alertView.isSupportRotating = YES;
                [alertView addButtonWithTitle:@"确定" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
                    SSExitVC *vc = [[SSExitVC alloc]init];
...
+ (void)checkVersion{
    [SSPublicNetWorkTool postGetAPPVersionWithSuccessBlock:^(ZLRequestResponse *responseObject) {
            ...
            if(arr.count>1){
                ...
                if([key isEqualToString:@"ios_version_check"]){
                    ...
                    if(kMeUnNilStr(version).length){
                        ...
                        NSArray * arrayCV = [kSSAppVersion componentsSeparatedByString:@"."];
                        if(arrayV.count==3 && arrayCV.count==3){
                            ...
                            //低于最低版本
                            if(vnumber>vcnumber){
                                SSAlertView *aler = [[SSAlertView alloc] initWithTitle:@"当前版本不可用" message:@"请升级"];
...
//    }
//    else{
//        [SSShowViewTool showMessage:@"已保存到手机相册" view:kMeCurrentWindow];
//    }
//}

//+ (void)checkVersion{
//    NSString * url = [[NSString alloc] initWithFormat:@"http://itunes.apple.com/lookup?id=%@",kSSAppId];
//    // 获取本地版本号
...
+ (void)getUIPasteboardContent{
    ...
    if(str.length){
        [SSPublicNetWorkTool postGoodsEncodeWithStr:str successBlock:^(ZLRequestResponse *responseObject) {
            if([responseObject.data isKindOfClass:[NSDictionary class]]){
                ...
                if(isVaile){
                    ...
                    [alertView addButtonWithTitle:@"立即前往" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
                        if (![kMeCurrentWindow.rootViewController isKindOfClass:[SSTabBarVC class]]) return;
                        pasteboard.string = @"";
                        SSTabBarVC *tabBarController = ( SSTabBarVC*)kMeCurrentWindow.rootViewController;
                        SSNavigationVC *nav = (SSNavigationVC *)tabBarController.selectedViewController;
                        UIViewController * baseVC = (UIViewController *)nav.visibleViewController;
                        SSThridProductDetailsVC *vc = [[SSThridProductDetailsVC alloc]initWithId:[product_id integerValue]];
                        vc.uid = uid;
...
//                    SSAlertView *aler = [[SSAlertView alloc] initWithTitle:@"提示" message:@"有来自好友的商品分享"];
...
//                    [aler show];
                    ...
                    [alertView show];
//                    SSAlertView *aler = [[SSAlertView alloc] initWithTitle:@"提示" message:@"有来自好友的商品分享,该链接已失效"];
//                    [aler addButtonWithTitle:@"确定" block:^{
//                        pasteboard.string = @"";
//                    }];
//                    [aler show];
