...
#define kSSAddTelViewHeight (288*kMeFrameScaleY())
#define kSSAddTelViewWidth (250*kMeFrameScaleX())
#define kBtnBottomMargin (35*kMeFrameScaleY())

@interface SSAddTelView ()<UITextFieldDelegate>{
...
@implementation SSAddTelView

- (void)awakeFromNib{
    [super awakeFromNib];
    _consViewHeight.constant =kSSAddTelViewHeight;
    _consViewWidth.constant =kSSAddTelViewWidth;
...
- (void)tfVerficationTextDidChange:(UITextField *)textField{
    if(textField.text.length > kLimitVerficationNum){
        [_btnAdd setBackgroundColor:kSSPink];
        _btnAdd.enabled = YES;
    }else{
...
- (IBAction)addTelAction:(UIButton *)sender {
    kMeWEAKSELF
    [SSPublicNetWorkTool postaddPhoneWithPhone:kMeUnNilStr(_tfNnumber.text) code:kMeUnNilStr(_tfCaptcha.text) invate:@"" successBlock:^(ZLRequestResponse *responseObject) {
        //绑定手机号成功 保存到本地
        kMeSTRONGSELF
...
- (IBAction)captchaAction:(UIButton *)sender {
    if(![SSCommonTool isValidPhoneNum:kMeUnNilStr(_tfNnumber.text)]){
        [SSShowViewTool showMessage:@"手机格式不对" view:self];
        _tfNnumber.text = @"";
        return;
    }
    kMeWEAKSELF
    [SSPublicNetWorkTool postGetCodeWithrPhone:kMeUnNilStr(_tfNnumber.text) type:@"2" successBlock:^(ZLRequestResponse *responseObject) {
        kMeSTRONGSELF
