...
@interface SSPosterMoreListContentVC ()<UICollectionViewDelegate,UICollectionViewDataSource,RefreshToolDelegate>{
...
@implementation SSPosterMoreListContentVC

- (instancetype)initWithModel:(SSPosterModel *)Model isNew:(BOOL)isNew{
    if(self = [super init]){
        _model = Model;
        _isNew = isNew;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SSPosterMoreListVC *homeVC = [SSCommonTool getVCWithClassWtihClassName:[SSPosterMoreListVC class] targetResponderView:self];
    if(homeVC){
...
- (ZLRefreshTool *)refresh{
    if(!_refresh){
        _refresh = [[ZLRefreshTool alloc]initWithContentView:self.collectionView url:kGetApiWithUrl(SSIPcommonFindPostersClass)];
