...
#import "SSPosterMoreListContentVC.h"
#import "SSPosterModel.h"

@interface SSPosterMoreListVC()<JXCategoryViewDelegate,UIScrollViewDelegate>{
    NSArray *_arrType;
...
@property (nonatomic, strong) SSPosterMoreListContentVC *newVC;
@property (nonatomic, strong) SSPosterMoreListContentVC *TopVC;

@end

@implementation SSPosterMoreListVC

- (instancetype)initWithModel:(SSPosterModel *)Model{
    if(self = [super init]){
        _model = Model;
    }
    return self;
...
- (SSPosterMoreListContentVC *)newVC{
    if(!_newVC){
        _newVC = [[SSPosterMoreListContentVC alloc]initWithModel:_model isNew:YES];
...
- (SSPosterMoreListContentVC *)TopVC{
    if(!_TopVC){
        _TopVC = [[SSPosterMoreListContentVC alloc]initWithModel:_model isNew:NO];
