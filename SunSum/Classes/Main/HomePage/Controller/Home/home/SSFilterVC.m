...
@interface SSFilterVC ()<UITableViewDelegate, UITableViewDataSource,RefreshToolDelegate>{
    SSFilterMainModel *_allModel;
}
...
@implementation SSFilterVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"商品分类";
    _allModel = [SSFilterMainModel new];
...
- (void)handleResponse:(id)data{
    ...
    [self.refresh.arrData addObjectsFromArray:[SSFilterMainModel mj_objectArrayWithKeyValuesArray:data]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section){
        SSFilterMainModel *model = self.refresh.arrData[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section){
        SSFilterMainModel *model = self.refresh.arrData[indexPath.row];
        SSFilterGoodVC *vc = [[SSFilterGoodVC alloc]initWithcategory_id:[NSString stringWithFormat:@"%@",@(model.idField)] title:kMeUnNilStr(model.category_name)];
        [self.navigationController pushViewController:vc animated:YES];
    }else{
        SSFilterGoodVC *vc = [[SSFilterGoodVC alloc]initWithcategory_id:[NSString stringWithFormat:@"%@",@(_allModel.idField)] title:kMeUnNilStr(_allModel.category_name)];
        [self.navigationController pushViewController:vc animated:YES];
    }
}
