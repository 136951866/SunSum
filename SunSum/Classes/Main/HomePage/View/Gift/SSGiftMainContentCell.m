...
@interface SSGiftMainContentCell (){
    SSShoppingCartModel *_goodsModel;
}
...
@implementation SSGiftMainContentCell
...
- (void)setUIWIthModel:(SSShoppingCartModel *)goodsModel{
...
- (IBAction)cut:(UIButton *)sender {
    ...
    if (count <= 0) {
        ...
        kMeWEAKSELF
        [SSPublicNetWorkTool posteditCartNumWithShopCartId:_goodsModel.product_cart_id num:count successBlock:^(ZLRequestResponse *responseObject) {
            kMeSTRONGSELF
...
- (IBAction)add:(UIButton *)sender {
    ...
    if(count>_goodsModel.stock){
        [SSShowViewTool showMessage:@"库存不足" view:kMeCurrentWindow];
        return;
    }
    kMeWEAKSELF
    [SSPublicNetWorkTool posteditCartNumWithShopCartId:_goodsModel.product_cart_id num:count successBlock:^(ZLRequestResponse *responseObject) {
