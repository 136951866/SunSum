...
@implementation SSCoupleMailCell

- (void)awakeFromNib {
    [super awakeFromNib];
    _lblJuan.adjustsFontSizeToFitWidth = YES;
    _consImgHeight.constant = kSSCoupleMailCellWdith;
    // Initialization code
}
...
- (void)setpinduoduoUIWithModel:(SSPinduoduoCoupleModel *)model{
    ...
    _lblOrigalPrice.text = [NSString stringWithFormat:@"原价¥%@", [SSCommonTool changeformatterWithFen:@(model.min_group_price)]];
     _lblJuan.text =[NSString stringWithFormat:@"%@元券",[SSCommonTool changeformatterWithFen:@(model.coupon_discount)]];
    _lblJuanPrice.text =[NSString stringWithFormat:@"¥%@", [SSCommonTool changeformatterWithFen:@(model.min_group_price-model.coupon_discount)]];
