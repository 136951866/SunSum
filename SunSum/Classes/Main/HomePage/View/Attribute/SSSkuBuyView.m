...
#import "SSPriceAndStockModel.h"

@interface SSSkuBuyView ()<UITableViewDelegate,UITableViewDataSource>{
    UIView *_superView;
    CGFloat _allHeight;
    BOOL _isInteral;
}
...
@property (nonatomic, strong) SSGoodDetailModel *goodModel;
//@property (nonatomic, weak) UITableView *superView;
@end

@implementation SSSkuBuyView

- (void)dealloc{
    NSLog(@"/*************** SSSkuBuyView  delloc *******************/");
}
...
//        获取第一个价格和库存
        [SSPublicNetWorkTool postPriceAndStockWithGoodsId:@(goodModel.product_id).description specIds:str ssuccessBlock:^(ZLRequestResponse *responseObject) {
            kMeSTRONGSELF
            if(strongSelf){
                strongSelf->_goodModel.spec_ids = str;
                SSPriceAndStockModel *psmodel = [SSPriceAndStockModel mj_objectWithKeyValues:responseObject.data];
                strongSelf->_goodModel.psmodel = psmodel;
                NSMutableArray *arrSpcName = [NSMutableArray array];
                [kMeUnArr(strongSelf->_goodModel.spec) enumerateObjectsUsingBlock:^(SSGoodDetailSpecModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {\
                    kMeSTRONGSELF
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SSSkuBuyCell *cell=[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([SSSkuBuyCell class]) forIndexPath:indexPath];
    kMeWEAKSELF
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return [SSSkuBuyCell getCellHeigthWithdetailModel:_goodModel];
}
...
- (UITableView *)tableView{
    if(!_tableView){
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([SSSkuBuyCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([SSSkuBuyCell class])];
