...
static char *const kSSFilterClerkViewKey = "SSFilterClerkViewKey";

@interface SSFilterClerkView()

@property (nonatomic, strong) UICollectionView *collect;

@end

@implementation SSFilterClerkView
...
- (void)initUI{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == _defaultSelectIndex) {
            ...
            objc_setAssociatedObject(button, kSSFilterClerkViewKey, @"1", OBJC_ASSOCIATION_ASSIGN);
        }else{
            button.selected = NO;
            [button setImage:[UIImage imageNamed:@"jiagenomal"] forState:UIControlStateNormal];
            [button setButtonImageTitleStyle:ButtonImageTitleStyleRight padding:2];
            objc_setAssociatedObject(button, kSSFilterClerkViewKey, @"0", OBJC_ASSOCIATION_ASSIGN);
        }
    }
}
...
- (void)selectClick:(UIButton *)btn{
    ...
    NSString *flag = objc_getAssociatedObject(btn, kSSFilterClerkViewKey);
    if ([flag isEqualToString:@"1"]) {
        [btn setImage:[UIImage imageNamed:@"jiageup"] forState:UIControlStateNormal];
        objc_setAssociatedObject(btn, kSSFilterClerkViewKey, @"2", OBJC_ASSOCIATION_ASSIGN);
        ...
        objc_setAssociatedObject(btn, kSSFilterClerkViewKey, @"1", OBJC_ASSOCIATION_ASSIGN);
