...
@interface SSGetCaseContentVC ()<UITableViewDelegate, UITableViewDataSource,RefreshToolDelegate>{
...
@implementation SSGetCaseContentVC
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    SSGetCaseModel *model = self.refresh.arrData[indexPath.row];
    return [SSGetCaseCell getCellHeightWithModel:model];
}
...
- (ZLRefreshTool *)refresh{
    if(!_refresh){
        _refresh = [[ZLRefreshTool alloc]initWithContentView:self.tableView url:kGetApiWithUrl(SSIPcommondestoonFinanceCashDetail)];
