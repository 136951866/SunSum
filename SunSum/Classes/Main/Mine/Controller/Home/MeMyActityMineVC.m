...
- (void)handleResponse:(id)data{
    ...
    [self.refresh.arrData addObjectsFromArray:[SSMineActiveModel mj_objectArrayWithKeyValuesArray:data]];
}
...
- (ZLRefreshTool *)refresh{
    if(!_refresh){
        _refresh = [[ZLRefreshTool alloc]initWithContentView:self.tableView url:kGetApiWithUrl(SSIPadminGetAppGetShare)];
        _refresh.delegate = self;
        _refresh.isDataInside = YES;
        [_refresh setBlockEditFailVIew:^(ZLFailLoadView *failView) {
            failView.backgroundColor = kSSeeeeee;
