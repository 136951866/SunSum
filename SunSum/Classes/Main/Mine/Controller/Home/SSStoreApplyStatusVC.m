...
@interface SSStoreApplyStatusVC ()
...
@implementation SSStoreApplyStatusVC
...
- (IBAction)reApplyAction:(UIButton *)sender {
    MBProgressHUD *hub =  [SSPublicNetWorkTool commitWithHUD:@"获取审核数据中.."];
    
    SSStoreApplyVC *vc = [[SSStoreApplyVC alloc]init];
    SSStoreApplyParModel *parModel = [SSStoreApplyParModel new];
    ...
    if(kMeUnNilStr(_model.business_images_url).length){
        ...
        SSBynamicPublishGridModel *business_imagesModel = [SSBynamicPublishGridModel modelWithImage:image isAdd:NO];
        business_imagesModel.filePath = [SSCommonTool getNoCompressImagePath:image filename: kMeUnNilStr(_model.business_images)];
        parModel.business_imagesModel = business_imagesModel;
        
    }else{
        SSBynamicPublishGridModel *business_imagesModel = [SSBynamicPublishGridModel modelWithImage:[UIImage imageNamed:@"icon_bynamicAdd"] isAdd:YES];
        parModel.business_imagesModel = business_imagesModel;
    }
    ...
    if(kMeUnNilStr(_model.mask_img_url).length){
        ...
        UIImage *image = [UIImage imageWithData:data];
        SSBynamicPublishGridModel *mask_imgModel = [SSBynamicPublishGridModel modelWithImage:image isAdd:NO];
        mask_imgModel.filePath = [SSCommonTool getNoCompressImagePath:image filename: kMeUnNilStr(_model.mask_img)];
        parModel.mask_imgModel = mask_imgModel;
        
    }else{
        SSBynamicPublishGridModel *mask_imgModel = [SSBynamicPublishGridModel modelWithImage:[UIImage imageNamed:@"icon_bynamicAdd"] isAdd:YES];
        parModel.mask_imgModel = mask_imgModel;
    }
    ...
    if(kMeUnNilStr(_model.mask_info_img_url).length){
        ...
        UIImage *image = [UIImage imageWithData:data];
        SSBynamicPublishGridModel *mask_info_imgModel = [SSBynamicPublishGridModel modelWithImage:image isAdd:NO];
        mask_info_imgModel.filePath = [SSCommonTool getNoCompressImagePath:image filename: kMeUnNilStr(_model.mask_info_img)];
        parModel.mask_info_imgModel = mask_info_imgModel;
        
    }else{
        SSBynamicPublishGridModel *mask_info_imgModel = [SSBynamicPublishGridModel modelWithImage:[UIImage imageNamed:@"icon_bynamicAdd"] isAdd:YES];
        parModel.mask_info_imgModel = mask_info_imgModel;
    }
...
//- (void)setModel:(SSStoreApplyModel *)model{
//    _model = model;
