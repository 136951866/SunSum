...
@interface SSOrderExpressDetailVC ()<UITableViewDelegate,UITableViewDataSource>{
    SSOrderDetailModel *_model;
}
@property (nonatomic, strong) UITableView *tableView;
@end

@implementation SSOrderExpressDetailVC

- (instancetype)initWithModel:(SSOrderDetailModel *)mode{
    if(self = [super init]){
        _model = mode;
    }
    return self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    SSexpressDetailModel *model = kMeUnArr(_model.express_detail)[indexPath.row];
    SSOrderExpressDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([SSOrderExpressDetailCell class]) forIndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    SSexpressDetailModel *model = kMeUnArr(_model.express_detail)[indexPath.row];
    return [SSOrderExpressDetailCell getCellHeightWithModel:model];
}
...
- (UITableView *)tableView{
    if(!_tableView){
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([SSOrderExpressDetailCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([SSOrderExpressDetailCell class])];
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        _tableView.showsVerticalScrollIndicator = NO;
        _tableView.tableFooterView = [UIView new];
        _tableView.delegate = self;
        _tableView.dataSource = self;
        _tableView.backgroundColor = kSSf4f4f4;
