...
@interface SSModifyPhoneVC ()<UITextFieldDelegate>{
...
@implementation SSModifyPhoneVC
...
- (IBAction)comFirmAction:(UIButton *)sender {
    kMeWEAKSELF
    [SSPublicNetWorkTool posteditPhoneWithPhone:kMeUnNilStr(kCurrentUser.mobile) code:kMeUnNilStr(_tfCaptcha.text) new_phone:kMeUnNilStr(_tfNnumber.text) successBlock:^(ZLRequestResponse *responseObject) {
        kMeSTRONGSELF
