...
+(SSBaseButton *)btnWithFrame:(CGRect)rect Img:(UIImage *)img{
    SSBaseButton *btn = [SSBaseButton buttonWithType:UIButtonTypeCustom];
    btn.frame = rect;
...
+(SSBaseButton *)btnWithFrame:(CGRect)rect Img:(UIImage *)img title:(NSString *)title target:(id)target Action:(SEL)act{
    SSBaseButton *btn = [self btnWithFrame:rect Img:img title:title];
...
+(SSBaseButton *)greenBtnWithFrame:(CGRect)rect title:(NSString *)title target:(id)target Action:(SEL)act{
    SSBaseButton *btn = [self btnWithFrame:rect Img:nil title:title target:target Action:act];
...
+(SSBaseButton *)btnWithImg:(UIImage *)img title:(NSString *)title target:(id)target Action:(SEL)act center:(CGPoint)ct{
    SSBaseButton *btn = [self btnWithFrame:CGRectMake(0, 0, img.size.width, img.size.height) Img:img title:title target:target Action:act];
    btn.center = ct;
    return btn;
}
...
+(SSBaseButton *)btnWithFrame:(CGRect)rect Title:(NSString *)title titleFont:(UIFont *)ft aboveImg:(UIImage *)img imgCenter:(CGPoint)ct_img target:(id)target Action:(SEL)act{
    SSBaseButton *btn = [self btnWithFrame:rect
                                  Img:nil
                                title:title
                               target:target
                               Action:act];
...
+(SSBaseButton *)btnWithTitle:(NSString *)title titleFont:(UIFont *)ft aboveImg:(UIImage *)img target:(id)target Action:(SEL)act{
    SSBaseButton *btn = [self btnWithFrame:CGRectMake(0, 0, img.size.width+[title sizeWithFont:ft].width+20, img.size.height) Title:title titleFont:ft aboveImg:img imgCenter:CGPointMake(img.size.width/2, img.size.height/2) target:target Action:act];
    return btn;
}
...
+(MBProgressHUD *)SHOWHUDWITHImgn:(NSString *)imgname test:(NSString *)text{
    ...
    [SSView SHOWHUDWITHHUD:HUD imgn:imgname test:text];
    return HUD;
}
