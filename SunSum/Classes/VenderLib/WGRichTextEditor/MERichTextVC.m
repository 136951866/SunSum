...
- (void)getHTMLText{
    ...
    kMeWEAKSELF
    MBProgressHUD *hub = [SSPublicNetWorkTool commitWithHUD:@"保存中"];
    ...
    [imagePicker setDidFinishPickingPhotosWithInfosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto, NSArray<NSDictionary *> *infos) {
        ...
        for (int i = 0; i < assets.count; i ++) {
            ...
            if (phAsset.mediaType == PHAssetMediaTypeImage) {
                ...
                NSString *filePath= [SSCommonTool getImagePath:image filename:filename];
                MBProgressHUD *HUD = [SSPublicNetWorkTool commitWithHUD:@"上传中"];
                [SSPublicNetWorkTool postQiNiuUpFileWithToken:strongSelf->_token filePath:filePath successBlock:^(id object) {
                    NSLog(@"%@",object);
                    kMeSTRONGSELF
