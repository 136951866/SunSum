...
- (void)onForceOffline:(NSNotification *)notification
{
    if ([SSUserInfoModel isLogin]){
        ...
        [alertView addButtonWithTitle:@"确定" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
            [SSWxLoginVC presentLoginVCWithIsShowCancel:NO SuccessHandler:nil failHandler:nil];
        }];
        [alertView show];
    }
}
...
-(void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler API_AVAILABLE(ios(10.0)){
    ...
    [UIApplication sharedApplication].applicationIconBadgeNumber +=1;
    if (![self.window.rootViewController isKindOfClass:[SSTabBarVC class]]) return;
    // 取到tabbarcontroller
    SSTabBarVC *tabBarController = (SSTabBarVC*)self.window.rootViewController;
...
- (void)applicationDidBecomeActive:(UIApplication *)application{
    ...
    [SSCommonTool getUIPasteboardContent];
    if ([SSUserInfoModel isLogin]) {
...
- (void)dealJPushActionWithDic:(NSDictionary *)userInfo{
    ...
    if([dict count]){
        ...
        SSJupshContentModel *model =  [SSJupshContentModel mj_objectWithKeyValues:contentDic];
        ...
        if(strType.length && [strType isEqualToString:@"1"] && [strType isEqualToString:@"2"]&& [strType isEqualToString:@"3"]&& [strType isEqualToString:@"7"]){
            ...
            [alertView addButtonWithTitle:@"立即前往" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
                if (![self.window.rootViewController isKindOfClass:[SSTabBarVC class]]) return;
                [SSPublicNetWorkTool getUserReadedNoticeWithNoticeId:model.msg_id SuccessBlock:^(ZLRequestResponse *responseObject) {
                    kNoticeUnNoticeMessage
                } failure:nil];
                // 取到tabbarcontroller
                SSTabBarVC *tabBarController = ( SSTabBarVC*)self.window.rootViewController;
                ...
                if([strType isEqualToString:@"1"]){
                    SSThridProductDetailsVC *dvc = [[SSThridProductDetailsVC alloc]initWithId:[kMeUnNilStr(model.idField) integerValue]];
                    [baseVC.navigationController pushViewController:dvc animated:YES];
                }else if ([strType isEqualToString:@"2"]){
                    SSMyOrderDetailVC *dvc = [[SSMyOrderDetailVC alloc]initWithOrderGoodsSn:kMeUnNilStr(model.idField)];
                    [baseVC.navigationController pushViewController:dvc animated:YES];
